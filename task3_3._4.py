...
def get_accuracy_per_length(labelled_constituents1, labelled_constituents2):
    dic = dict()
    for index in range(len(labelled_constituents1)):
        ...
        for index2 in range(len(labelled_constituent1)):
            try:
                length = labelled_constituent2[index2][2] - labelled_constituent2[index2][1] + 1
                if str(length) not in dic:
                    dic[str(length)] = [0, 1]
                else:
                    dic[str(length)][1] += 1
...
def get_accuracy_per_label(labelled_constituents1, labelled_constituents2):
    dic = dict()
    for index in range(len(labelled_constituents1)):
        labelled_constituent1 = labelled_constituents1[index]
        labelled_constituent2 = labelled_constituents2[index]
        for index2 in range(len(labelled_constituent1)):
            try:
                label = labelled_constituent2[index2][0]
                if label not in dic:
                    dic[label] = [0, 1]
                else:
                    dic[label][1] += 1
...
# this is just for 3.3:
'''
def turn_dic_into_xs_ys(dic):
    xs = list()
    ys = list()
...
dic = get_accuracy_per_length(list_of_labelled_constituents_for_gen_trees, list_of_labelled_constituents_for_trees)
x_values, y_values = turn_dic_into_xs_ys(dic)

plt.plot(x_values, y_values)
plt.show()
'''

# this is just for 3.4
dic = get_accuracy_per_label(list_of_labelled_constituents_for_gen_trees, list_of_labelled_constituents_for_trees)
for k, v in dic.items():
    print('for this nt: {} i got the following ratio: {}'.format(k, v[0] / v[1]))
